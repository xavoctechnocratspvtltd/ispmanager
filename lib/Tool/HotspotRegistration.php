...
class Tool_HotspotRegistration extends \xepan\cms\View_Tool{
	...
	function init(){
		...
		if($this->options['login_page']){
			$registration_form->layout->template->trySet('login_page',$this->app->url($this->options['login_page']));
		}

		if($this->options['forgot_password_page']){
			$registration_form->layout->template->trySet('forgot_password_url',$this->app->url($this->options['forgot_password_page']));
		}

		$registration_form->addSubmit("Sign Up")->addClass('btn btn-success btn-lg text-center btn-block');
		...
		// if($this->options['forgot_password_page']){
		// 	$registration_form->layout->template->trySet('forgot_password_page',$this->app->url($this->options['forgot_password_page']));
		// }
