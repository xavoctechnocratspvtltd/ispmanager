...
-- ----------------------------
--  Procedure structure for `updateAccountingData`
-- ----------------------------
DROP PROCEDURE IF EXISTS `updateAccountingData`;
delimiter ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateAccountingData`(dl_data bigint, ul_data bigint, now datetime, username varchar(255))
...
IF @download_data_consumed + @upload_data_consumed + @user_SessionInputOctate + @user_SessionOutputOctate > @data_net_data_limit THEN
	SET @fup = true;
	IF @treat_fup_as_dl_for_last_limit_row THEN 
		CALL getApplicableRow(username,now,TRUE,@data_applicable_row_id);
...
IF @dl_limit is null AND @ul_limit is null THEN
	SET @access = false;
END IF;
...
RETURN CONCAT(@access,',', @coa,',', @dl_limit,',', @ul_limit);
