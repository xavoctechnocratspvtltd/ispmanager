...
delimiter ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `checkAuthenticationReadOnly`(now datetime, username varchar(255)) RETURNS text CHARSET utf8
    READS SQL DATA
P:BEGIN
...
delimiter ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `sessionClose`(username varchar(255)) RETURNS text CHARSET utf8
    MODIFIES SQL DATA
BEGIN
	UPDATE 
		isp_user_plan_and_topup 	
